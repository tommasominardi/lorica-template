...
// local file
	...
	if(kGetMediaMimeType()=="video/x-flv"||kGetMediaMimeType()=="video/f4v")
	{
		...
		<?
	} elseif(substr(kGetMediaMimeType(), 0, 5) == "video") {
		?>
		<video width="<?= kGetMediaWidth(); ?>" height="<?= kGetMediaHeight(); ?>" controls>
			<source src="<?= kGetMediaUrl(); ?>" type="<?= kGetMediaMimeType(); ?>">
			Your browser does not support the video tag.
		</video>
		<?
	} elseif(substr(kGetMediaMimeType(), 0, 5) == "audio") {
		?>
		<audio controls>
			<source src="<?= kGetMediaUrl(); ?>" type="<?= kGetMediaMimeType(); ?>">
			Your browser does not support the video tag.
